...
contract BNBmonster is Context, IERC20, Ownable {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 WAX = tAmount.mul(_teamShare).div(
            _totalDistributionShares
        );

        _basicTransfer(address(this), WAXAddress, WAX);
        if(tokensForBurn>0){_basicTransfer(address(this), deadAddress, tokensForBurn);
        }

        uint256 leftover = tAmount - tokensForBurn - WAX;
